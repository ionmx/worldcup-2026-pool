...
def build_matches(data)
  ...
  data.each do |item|
    ...
    home = item.dig('Home', 'Abbreviation') ? item.dig('Home', 'Abbreviation') : item.dig('PlaceHolderA')
    homeName =  item.dig('Home', 'ShortClubName') ? item.dig('Home', 'ShortClubName') : item.dig('PlaceHolderA')
    away = item.dig('Away', 'Abbreviation') ? item.dig('Away', 'Abbreviation') : item.dig('PlaceHolderB')
    awayName =  item.dig('Away', 'ShortClubName') ? item.dig('Away', 'ShortClubName') : item.dig('PlaceHolderB')
    ...
    hash["#{game}"] = 
     {
        ...
        locationCity: item.dig('Stadium', 'CityName', 0, 'Description'),
        locationCountry: item.dig('Stadium', 'IdCountry'),
        home:,
        homeName:,
